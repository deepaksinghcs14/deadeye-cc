...
longer needs a release.** v0.41.0 made the kernel's two ceilings
catalog-driven (`unsure_ceiling`/`high_ceiling` role tags), but the only
catalog every install actually read still shipped compiled into the
binary -- the new capability existed but nothing could reach it without a
plugin update. Closed the loop: `docs/site/catalog.json`, generated by
`scripts/gen-catalog.go` from the same seed table as the compiled-in one,
is now published on the project's existing GitHub Pages site. Every
install refreshes a local cache of it in the background (`mode.catalog_check`,
on by default, ~24h TTL, a single read-only GET, mirrors the existing
release-tag check exactly) and `catalog.Load()` prefers it over the
compiled-in table when present. A model or price change is now: edit the
JSON, push to main, done -- no tag, no goreleaser, no reinstall.

Safety is layered so a bad publish can't reach routing: a fetched catalog
is only used if it passes a new `Catalog.Valid()` (real prices, unique
tiers, a tier 0, at most one of each ceiling role, and the roles ordered
correctly if both are present) and isn't older than the compiled-in
table; anything else falls straight back to builtin. A user's own
`~/.deadeye/catalog.json` override still wins outright and is never
touched by the refresh. Verified live: promoting `fable-5` to
`high_ceiling` via a hand-written cache file made it reachable for
high-complexity evidence with zero code change and zero release, a
corrupted cache fell back to builtin instead of half-applying, and an
existing override survived a refresh byte-for-byte.
