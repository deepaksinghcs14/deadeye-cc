...
// Codex and Gemini CLI are REDUCED hosts -- no subagent/Agent-tool surface
...
// HasSubagentSurface reports whether host exposes the Agent/subagent tool
// that deadeye's model routing, tier guidance, and workflow hint target.
// Only Claude Code does; Codex and Gemini have no subagent surface, so
// those injections are omitted there.
